...
ソフトウェア開発は、要求の理解から設計・実装・テストに至るまで多くの工程を必要とし、特に不慣れな技術領域や曖昧な要求に対応する場合、工数が膨らみやすい。本プロジェクトでは、仕様（Specification）を開発プロセスの中心に据え、AIと人間が協働するアプローチを採用した結果、**総工数を約73%削減**（55人月相当 → 15人月、**人件費換算で約 6,667 万円の圧縮**）するとともに、高い品質水準を実現した。AI 利用料は標準シナリオで約 13 万円にとどまり、**ROI は約 511 倍**に達した。
...
工数削減効果が大きいとはいえ、AI 利用そのものにも費用がかかる。本節では、GitHub Copilot Enterprise の料金体系（2026年5月時点）[10]と人件費単価を用いて、AI 投資額と人件費削減額を貨幣換算で比較する。

| 項目         | 値                                               | 根拠・出典           |
| ------------ | ------------------------------------------------ | -------------------- |
| 人件費単価   | 2,000万円/年人（≒ 167万円/人月）                 | 企業負担総コスト前提 |
| 為替レート   | 1 USD = 158円                                    | 2026年5月概算        |
| Copilot 月額 | $39/user/月（プレミアムリクエスト1,000回込み）   | GitHub Docs[10]      |
| 超過料金     | $0.04/premium request                            | GitHub Docs[10]      |
| 開発期間     | 3名 × 5ヶ月 = 15人月 = 約300人日                 | 報告書2.3節実績      |
| モデル構成   | Sonnet 4.6 : 50% / GPT-4o : 20% / Opus 4.6 : 30% | 本プロジェクト実績   |
| 加重平均乗数 | 0.5×1.0 + 0.2×0 + 0.3×3.0 = **1.4x**             | モデル別乗数より算出 |

**モデル別のプレミアムリクエスト乗数**：

| モデル            | 乗数 | 用途                                 |
| ----------------- | ---- | ------------------------------------ |
| GPT-4o            | 0x   | 軽量タスク（込み込みで追加料金なし） |
| Claude Sonnet 4.6 | 1.0x | 標準的なコード生成・レビュー         |
| Claude Opus 4.6   | 3.0x | 複雑な設計判断・困難なデバッグ       |

#### 6.5.2 AI 利用料金の見積もり

1日あたりのAI対話数を3シナリオで試算した。総対話数は「3名 × 20日/月 × 5ヶ月」で算出し、加重平均乗数 1.4x を乗じてプレミアムリクエスト消費量を求める。

| シナリオ | 対話/日/人 | 総対話数 | 必要premium req (×1.4倍) | 含まれる枠 | 超過分 | 超過料金 (USD) | サブスク (USD) | 合計 (USD) | **合計（円）** |
| -------- | ---------- | -------- | ------------------------ | ---------- | ------ | -------------- | -------------- | ---------- | -------------- |
| 保守的   | 30         | 9,000    | 12,600                   | 15,000     | 0      | $0             | $585           | $585       | **約9.2万円**  |
| 標準     | 50         | 15,000   | 21,000                   | 15,000     | 6,000  | $240           | $585           | $825       | **約13.0万円** |
| 積極     | 80         | 24,000   | 33,600                   | 15,000     | 18,600 | $744           | $585           | $1,329     | **約21.0万円** |

- サブスクリプション基本料金: $39 × 3人 × 5ヶ月 = $585
- 含まれる premium request: 1,000/月/人 × 3人 × 5ヶ月 = 15,000回
- 保守シナリオでは含まれる枠内に収まり、超過料金は発生しない

#### 6.5.3 人件費削減額

6.1節で示した工数（想定55人月 → 実績15人月）を貨幣換算する。

| 項目                | 人月       | 金額                           |
| ------------------- | ---------- | ------------------------------ |
| AI 未使用時（想定） | 55人月     | 55 × 167万 = **約 9,167 万円** |
| AI 使用時（実績）   | 15人月     | 15 × 167万 = **約 2,500 万円** |
| **人件費削減額**    | **40人月** | **約 6,667 万円**              |

#### 6.5.4 ROI の算出

3シナリオそれぞれで、AI 投資額と人件費削減額の対比を整理する。

| シナリオ | AI 投資額 | 純削減額（削減額 − AI 投資） | **ROI 倍率**  | AI 投資の人件費比率 |
| -------- | --------- | ---------------------------- | ------------- | ------------------- |
| 保守的   | 9.2 万円  | 約 6,658 万円                | **約 721 倍** | 約 0.10%            |
| 標準     | 13.0 万円 | 約 6,654 万円                | **約 511 倍** | 約 0.14%            |
| 積極     | 21.0 万円 | 約 6,646 万円                | **約 317 倍** | 約 0.23%            |

![図9: 人件費削減額 vs AI 投資額の対比](figs/cost_benefit.png)

**図9: 人件費削減額 vs AI 投資額の対比（標準シナリオ）**

#### 6.5.5 考察

本試算から、以下の3点が導かれる。

1. **AI 投資は人件費に対して 0.2% 未満**：標準シナリオで AI 利用料は 13 万円程度であり、AI 未使用時の人件費（9,167 万円）と比較すると 0.14% にすぎない。これは「AI 投資を出し惜しむことで開発工数を増やす」判断が経済合理性を欠くことを示す。

2. **保守的シナリオでも ROI は 700 倍超**：1日あたりの対話数を控えめに見積もっても、AI 投資 1 円あたり 700 円超の人件費削減効果が見込まれる。投資判断において感度分析の下限値が依然として圧倒的である点は、本手法の頑健性を示す。

3. **モデル選定の経済的影響**：Opus 4.6（乗数 3x）は Opus 4.7（乗数 15x[10]）と比較してコスト効率が5倍高い。同等のタスクで Opus 4.7 を 30% 使用した場合、加重平均乗数は 5.0x となり、AI 投資額は標準シナリオで約 47 万円まで上昇する。それでも人件費削減額の 0.7% にすぎないが、複雑性の低いタスクには軽量モデルを選ぶ運用ルールが ROI をさらに高める。

**試算の限界**：

- 1日あたり対話数は経験則ベースの仮定であり、実プロジェクトでの Copilot 利用ログがあれば実測値で更新すべきである
- モデル乗数は GitHub の料金改定により変動する。本試算は 2026年5月時点の値に基づく
- 人月単価は「企業負担総コスト（給与＋諸経費）」を想定。請負単価や派遣単価とは意味が異なる

> **脚注**: GitHub は 2026年6月1日 より、プレミアムリクエスト方式から AI Credit（トークン従量課金）方式へ料金体系を移行する予定である[10]。本試算はプロジェクト実施期間（旧方式）の費用として算出した。本手法を他プロジェクトに横展開する場合は、新料金体系での再試算が必要となる。
...
10. GitHub, "About billing for Copilot requests", https://docs.github.com/en/copilot/concepts/billing/copilot-requests （2026年5月時点）
